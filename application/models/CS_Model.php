...
class CS_Model extends CI_Model {
    ...
    function cekKamus($kata){
        // cari di database	
        $data = $this->db->get_where('tb_katadasar', array('katadasar' => $kata));
    ...
    function Cek_Rule_Precedence($kata){
        if(preg_match('/^(be)[A-Za-z]+(lah|an)$/',$kata)){ // be- dan -i
            return true;
        }
        if(preg_match('/^(di|([mpt]e)).*(i)$/',$kata)){ // di- dan -an				
            return true;	
        }
        return false;
    }
    ...
    function Cek_Prefix_Disallowed_Sufixes($kata){
        if(preg_match('/^(be)[A-Za-z]+(i)$/',$kata)){ // be- dan -i
            return true;
        }
        if(preg_match('/^(di)[A-Za-z]+(an)$/',$kata)){ // di- dan -an				
            return true;
            
        }
        if(preg_match('/^(ke)[A-Za-z]+(i|kan)$/',$kata)){ // ke- dan -i,-kan
            return true;
        }
        if(preg_match('/^(me)[A-Za-z]+(an)$/',$kata)){ // me- dan -an
            return true;
        }
        if(preg_match('/^(se)[A-Za-z]+(i|kan)$/',$kata)){ // se- dan -i,-kan
            return true;
        }
        return false;
    }
    ...
    function Del_Derivation_Prefix($kata){
        ...
        if(preg_match('/^([tmbp]e)\S{1,}/',$kata)){ //Jika awalannya adalah “te-”, “me-”, “be-”, atau “pe-”
            ...
            if(preg_match('/^(be)\S{1,}/',$kata)){ // Jika awalan “be-”,
                ...
                if(preg_match('/^(ber)[^aiueor][A-Za-z](?!er)\S{1,}/',$kata)){ //aturan  2.
                    $__kata = preg_replace('/^(ber)/','',$kata);
                    if($this->cekKamus($__kata)){			
                        return $__kata; // Jika ada balik
                    }
                ...
                if(preg_match('/^(ber)[^aiueor][A-Za-z]er[aiueo]\S{1,}/',$kata)){ //aturan  3.
                    $__kata = preg_replace('/^(ber)/','',$kata);
                    if($this->cekKamus($__kata)){			
                        return $__kata; // Jika ada balik
                    }
            ...
            if(preg_match('/^(pe)\S{1,}/',$kata)){ // Jika awalan “pe-”,
                ...
                if(preg_match('/^(per)[^aiueor][A-Za-z](?!er)\S{1,}/',$kata)){ // aturan  23
                    $__kata = preg_replace('/^(per)/','',$kata);
                    if($this->cekKamus($__kata)){			
                        return $__kata; // Jika ada balik
                    }
                ...
                if(preg_match('/^(per)[^aiueor][A-Za-z](er)[aiueo]\S{1,}/',$kata)){ // aturan  24
                    $__kata = preg_replace('/^(per)/','',$kata);
                    if($this->cekKamus($__kata)){			
                        return $__kata; // Jika ada balik
                    }
